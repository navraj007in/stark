...
pub fn build_and_link(
    ...
    // WP-C6.4c's hand-authored lock rests on a premise A10 breaks: "stark-runtime is
    // dependency-free, so the whole graph is two path-only packages and the lock is fully
    // determined by the generated crate itself". A provider crate has its own path dependencies
    // (stark-time-native needs stark-provider-abi), so the compiler can no longer author a correct
    // lock without performing dependency resolution -- which is Cargo's job, not the backend's.
    //
    // For a provider-free program the hand-authored lock is unchanged. With providers, Cargo
    // resolves the lock ONCE, offline, and the build below still passes `--locked`, so the
    // assertion that survives is the one that matters: the build must not alter the lock it was
    // given. What is given up is authorship of the lock, not the guarantee.
...
///
/// Used only when the program links providers. `--offline` keeps the no-network property that
/// §11.3 requires; the resolution itself is Cargo's because the provider graph is Cargo's to know.
...
fn read_runtime_version(runtime_crate: &Path) -> Result<String, BackendDiagnostic> {
    read_crate_version(runtime_crate)
}
