...
impl Server {
    ...
    fn handle_message<W: Write>(&mut self, message: &Message, writer: &mut W) -> std::io::Result<bool> {
        match message {
    ...
    fn handle_notification<W: Write>(&mut self, notif: &Notification, _writer: &mut W) -> std::io::Result<bool> {
            ...
            if let Some(changes) = params.get("contentChanges").and_then(|v| v.as_array()) {
                if !changes.is_empty() {
                    if let Some(text) = changes[0].get("text").and_then(|v| v.as_str()) {
                        self.state
                            .update_document(uri.to_string(), version as i32, text.to_string());
                        self.compile_document(uri);
                    }
                }
            }
    ...
    fn compile_document(&mut self, uri: &str) {
        if let Some(doc) = self.state.get_document(uri).map(|d| d.clone()) {
            ...
            // Parse
            let (ast, mut diagnostics) = parse_with_options(&source, ParseMode::Program, options.clone());

            let mut type_tables = None;

            // Try to resolve names if no parse errors
            if diagnostics.iter().all(|d| d.severity != crate::diag::Severity::Error) {
                let source_arc = Arc::new(source);
                let (hir, resolve_diags) = resolve_with_options(&ast, source_arc.clone(), options.clone());
    ...
    fn handle_hover(&mut self, id: i64, params: &JsonValue) -> Response {
        if let (Some(uri), Some(line), Some(character)) = (
            params
                ...
                .and_then(|v| v.get("uri"))
                .and_then(|v| v.as_str()),
            params.get("position").and_then(|v| v.get("line")).and_then(|v| v.as_i64()),
            params
                .get("position")
    ...
    fn handle_definition(&mut self, id: i64, params: &JsonValue) -> Response {
        if params.get("textDocument").and_then(|v| v.get("uri")).is_some() {
            return Response {
                id,
                result: Some(JsonValue::Null),
                error: None,
