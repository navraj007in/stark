...
///
/// Returns the source, the call bindings, and the raw-error variant table.
type Layer = (
    String,
...
fn raw_layer() -> Layer {
    ...
    // The status vocabulary is taken from the REGISTRY, not invented here. Lowering reads the
    // resolved call's binding, which comes from the same manifest, so a vocabulary stated
    // independently by the fixture can disagree with it -- and did: once `stark-time` declared
    // `ClockUnavailable` and `OutOfRange`, an empty fixture vocabulary produced no error mapping
    // while the resolved call declared two recoverable codes, and lowering refused the mismatch
    // rather than emitting `Ok` for a declared failure. Reading one source keeps them in step.
    let status_binding = provider.status_binding.clone();
    let decl = provider
        .metadata
        .functions
        .clone()
    ...
    // `RawTimeError` is generated by synthesis from that vocabulary, so the program below does not
    // declare the type itself -- the compiler owns it. `stark-time` now declares two recoverable
    // codes, so the generated enum is inhabited and every declared code must appear in it: a code
    // the provider can return with no variant to become is a failure the package cannot handle.
    let vocabularies = BTreeMap::from([(capability.clone(), status_binding.clone())]);
    let layer = synthesize(&[sig], &vocabularies).expect("the clock layer must synthesize");
    for (_, package_error) in status_binding.declared_codes() {
        let variant = package_error
            .rsplit("::")
            .next()
            .expect("a package error names a variant");
        assert!(
            layer.source.contains(variant),
            "every declared status must reach the generated raw error type; {variant} did not:\n{}",
            layer.source
        );
    }
