...
//!
//! **Where the machinery now lives (WP-C6.5 commit 2, finding C65-F1).** The engine runners, the
//! normalisation and the comparator moved verbatim to `tests/support/differential.rs` — this file
//! keeps the case declarations, the comparator's own negative tests, and nothing else. The move
//! was made because 23 test files ran all three engines against 23 private notions of "agree";
//! that module is now the single authority, and the remaining 22 suites adopt it in coverage-matrix
//! order. Nothing about this file's behaviour changed in that commit.
//!
