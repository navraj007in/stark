...
fn agree(tag: &str, src: &str, expect: &str) {
    ...
    assert_eq!(hir_exec.output, expect, "{tag}: HIR output");
    ...
    assert_eq!(mir_exec.output, expect, "{tag}: MIR output");

    if rustc_available() {
        let verified = verify_program(&program).unwrap();
        let dir = std::env::temp_dir().join(format!("stark_c63ef32_{tag}_{}", std::process::id()));
