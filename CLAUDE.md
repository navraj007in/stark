...
the compiler there are 30 first-party packages under `packages/`, capability-declared host access
...
- Packages and host access: **29 libraries plus the `stark-get` application live under
...
- Distribution: Installer Phase I is implemented — release archives, platform installers, a
  ...
  manifest verification. The payload carries 29 explicitly marked first-party libraries and all
...
platforms) and compiler-backed language services done, over a qualified subset. 30 first-party
