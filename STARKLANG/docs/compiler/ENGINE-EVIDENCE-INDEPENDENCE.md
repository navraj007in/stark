...
| `EV-SPEC-FIXTURES` — `spec fixture conformance` | front end | **the specification**, hand-triaged into `manifest.toml` | `SPEC_DERIVED` | **LOW** | the strongest FRONT-END control, and the only spec-derived one | Covers parse/semantic classification, not runtime semantics. **AS8 narrowed this further and by measurement (AS8-R5): it classifies programs by what the front end ACCEPTS AND REJECTS, so it does not control type IDENTITY either.** AS8-MUT-013 reverted TYPE-PRIM-001 — `()` no longer canonicalising to `Unit` — with this suite in the selection, and it passed. No program fails to compile, so no fixture can notice |
...
| `EV-SAMPLES` — `External sample suite (pinned)` | native | **external repository**, pinned by commit SHA with a resolution check | `EXTERNALLY_DERIVED` | **LOW** | the strongest END-TO-END control (this audit originally gave the same superlative to `EV-SPEC-FIXTURES`; they control different things and only one can hold the title unqualified) | Covers whole-application behaviour, not specific authorities; a shared-authority defect would surface only if a sample happens to exercise it |
...
For the front end, yes (`EV-SPEC-FIXTURES`) — **but only for acceptance and rejection.** AS8-MUT-013
showed it does not reach type identity even within the front end. For runtime semantics, **no**.
