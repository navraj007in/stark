...
| `ESF-DROP-003` | Whether destroying a value of a given `MirTy` **runs anything** | `SHARED_PREDICATE` | INVISIBLE mir↔native; potentially visible against hir | **AC4-F4.** Struct-recursion, `HostResource` and tuple arms all mutation-killed; a **built-in owning type** killed only incidentally. The comparator observes drops by a frame a case emits from its own `Drop` impl, and a built-in has none — the event is **structurally unobservable**, not merely inherited. **AC4-F4 CLOSED 2026-08-12 by owner disposition.** `ac4_builtin_destruction` controls the lowering DECISION and is accepted as sufficient for the authority claim. **Native built-in deallocation stays an explicit assurance residual and must not be described as directly observed** — the drop log cannot see it, the Miri lane runs `-Zmiri-ignore-leaks`, and a leak harness is future assurance work, not WP-ARCH-CLOSE work. **high → medium** |
...
| `ESF-RES-002` | Resource **close/release selection** | `SHARED_LOWERING` | INVISIBLE mir↔native | **AC4-F6 CLOSED 2026-08-12 by owner disposition.** The package/provider qualification lane is accepted as the authoritative control — acquire → use → explicit-and-drop release against a live TLS peer, **semantically stronger** than a synthetic unit test. Standing requirement: it remains a REQUIRED closure/release input for resource-lifecycle changes. No duplicate local test is added. **medium** |
