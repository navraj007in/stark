...
| C6.1f-b1 | A | Claude | (post-C6.1f-a) | `mir/lower.rs` (lease); `native_c61f_reborrow.rs` (new) | SHARED-CONTRACTS v1 | none (existing MIR ops; no verifier/version change) | `native_c61f_reborrow.rs` (5) | none — no new deviation | §7J:3 | **CLOSED (CD-090)** |
| C6.1f-b2 | A | Claude | (post-b1) | `mir/lower.rs`, `borrowck.rs` (lease); `native_c61f_b2_weakening.rs` (new) | SHARED-CONTRACTS v1 | none (existing MIR ops) | `native_c61f_b2_weakening.rs` (5) | none | §7J:3 | **CLOSED for 5 of 6 boundaries (CD-092)** — aggregate fields outstanding; array→slice moved to C6.3b |
| C6.1f-b3…b5 | A | Claude | (post-b1) | per the matrix §7 split | SHARED-CONTRACTS v1 | none yet (CE3 if MIR/verifier contract; CE4 if runtime/ABI) | native regressions | — | §7J:3 | **OPEN** |
...
| `mir/lower.rs`, `borrowck.rs` | A | C6.1f-b2: expected-type reference weakening (`&mut T` → `&T`) — `weaken_ref_to` in lowering; argument re-borrow in borrowck. | post-C6.1f-b1 | narrow: one new private helper + four boundary call sites; one borrowck arm | `native_c61f_b2_weakening.rs` | C6.1f-b2 | C6.1f-b2 (landed) |
| `mir/lower.rs` | A | C6.1f-b1: receiver-position auto-deref + re-borrow at the method's mutability (TYPE-METHOD-002). | post-C6.1f-a | narrow: one match arm rewritten to use the existing `lower_place_autoderef` | `native_c61f_reborrow.rs` | C6.1f-b1 | C6.1f-b1 (landed) |
