...
| Layout queries | Exact values from the selected versioned `TargetLayout` contract for every C5-supported `MirTy`, identical in all three engines (CD-067) |
...
8. Rust layout is backend-, target-, and toolchain-version dependent, as permitted by Core’s target-layout query contract. **It is not what a layout query answers** — CD-067: `size_of`/`align_of` come from the selected versioned `TargetLayout` contract, and the generated representation is unobservable and need not equal it.
...
**AMENDED by CD-067 (2026-07-23).** The original text here said the backend emits a compile-time
Rust query against the canonical generated representation (`core::mem::size_of::<GeneratedTy>() as
u64`). That is now **forbidden**, because it makes an observable language answer depend on one
transitional backend and on `repr(Rust)`'s deliberately unspecified field ordering. The replacement
rule:

> `size_of<T>` and `align_of<T>` return values from the selected versioned STARK `TargetLayout`
> contract. HIR, MIR and native execution consume that same contract. A backend's internal physical
> representation is not observable and need not equal those values unless a separate representation
> contract explicitly requires equivalence.

The backend therefore emits the contract's value as a constant:

```rust
4u64    // size_of::<Int32>() under target contract `stark-64-v1`
8u64    // align_of::<Int64>()
```

The generated crate asserts nothing about its own physical layout, and generated nominals are NOT
`#[repr(C)]`: a host-layout cross-check would enforce a rule Core v1 does not have. A non-normative
`--audit-host-layout` diagnostic may later REPORT differences without rejecting a conforming build.
