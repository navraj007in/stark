...
| **S10** | Dependency containment is **real and verified**: `package.rs:1087` refuses a dependency resolving outside the workspace. A same-day "correction" claiming otherwise was WITHDRAWN — it was read from a parallel session's unmerged branch that had checked out under me | — | — | `C10-THREAT-MODEL.md` §2a keeps both records. The rule it produced: **"read the code" is not a method until you establish WHICH code** |
