...
- **STARK Icons** file icon theme: `.stark` files get their own icon. Because a
  VS Code icon theme replaces the whole icon set rather than adding to it, the
  theme is the built-in Seti theme plus the one `.stark` entry, so every other
  file keeps its usual icon. Generated by `tools/build-icon-theme.py`.
